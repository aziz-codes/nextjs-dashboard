...
const Sidebar = () => {
 ...
 const {activeMenu} = useStateContext();
  ...
  return (
  <div>{activeMenu ? <div className='relative h-screen shadow-2xl md:shadow-xl
  w-64 border border-t-white flex flex-col items-start 
  divide-y-2 flex-wrap ' style={{zIndex:'10000'}}>

{/* Logo and brand section */}
 <div className='flex flex-row  justify-center w-28 h-16
...
<div className='relative w-full '>
      {links.map((item,index)=>(
          <div key={index} className='flex flex-row items-center gap-3 m-4
